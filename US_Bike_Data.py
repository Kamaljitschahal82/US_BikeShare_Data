...
def get_city():
    ...
    city = input('\nHello! Let\'s explore some US bikeshare data!\n'
                 'Would you like to see data for Chicago, New York, or Washington?in lower case\n')
    city = city.lower()
    if city == 'chicago':
        return chicago
    elif city == 'new york':
        ...
        print('\nCity is not valid & please use correct city name -chicago, New York, or Washington? ')
        return get_city()
...
def get_day():
    ...
    Returns:
        TODO: fill out return type and description (see get_city for an example)
    '''
    day = input('\nWhich day? Please type your response as an integer.example :01 or 1\n').lower()
    # TODO: handle raw input and complete function
...
def popular_stations(city_file, time_period):
    ...
    with open(filename,newline='\n') as csvfile:
        d = csv.DictReader(csvfile)
        for row in d:
            starts = row['Start Station']
            ends = row['End Station']
            freq[starts] += 1
            freq[ends] += 1
        ...
        for row in di: 
          try:  
               ...
               freq[birth] += 1
                ...
                print("NO BIRTHDAY DATA for Washington")
                return 'none'
        c = freq.most_common()
        for item in c:
            if item[0] != '':
